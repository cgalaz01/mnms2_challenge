...
class model:
    ...
    def test_prediction(self, model: Optional[tf.keras.Model] = None,
                        input_directory: Optional[Union[str, Path]] = None,
                        output_directory: Optional[Union[str, Path]] = None) -> None:
        if model is None and input_directory is None and output_directory is None:
            raise ValueError("Either 'model' must be provided, or both 'input_directory' and 'output_directory' must be provided.")
...
if __name__ == '__main__':
    ...
    m.predict(None, input_folder, output_folder)
